...
const Sidebar = () => {
  ...
  w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw]
   transition-all duration-300 z-20 px-4 lg:px-[35px] `}>
    ...
    <div className='flex flex-col gap-y-2 h-[300px] lg:h-[550px] overflow-y-auto overflow-x-hidden border-b'>
      {cart.map((item) =>{
    ...
    <div className='flex flex-col gap-y-4 py-4 mt-4'>
      <div className='flex w-full justify-between items-center'>
        ...
        {/* clear cart icon */}
        <div onClick={clearCart} className='cursor-pointer py-4 bg-red-500 text-white w-12 h-12 flex justify-center items-center text-xl rounded-xl'>
          <FiTrash/>
          </div>
      </div>
      <Link to='/' className='bg-gray-200 flex p-4 justify-center items-center text-primary w-full font-medium rounded-md'>View Cart</Link>
