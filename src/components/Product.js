     ...
     <div>
      <div className='border border-[#e4e4e4] h-[300px] mb-4 relative overflow-hidden group transition rounded-2xl'>
        <div className='w-full h-full flex justify-center items-center'>
          ...
          <img className='max-h-[200px] group-hover:scale-110 transition duration-300 flex justify-center ' src={image} alt="" />
        </div>
        </div>
       {/* button */}
       <div className='absolute top-6 -right-11 group-hover:right-5 p-2 flex flex-col items-center justify-center gap-y-2 opacity-0 group-hover:opacity-100 transition-all'>
        ...
        <Link to={`/product/${id}`} className="w-12 h-12 bg-white flex justify-center items-center text-primary drop-shadow-xl rounded-xl"><BsEyeFill/></Link>
       </div>
      </div>
